...
defaults = {
    ...
    "suite_d_cc": 0.0,
    "suite_d_oth": 0.0,
    # Separate independent state variables for Viber/WhatsApp Linker
    "viber_client_name": "",
...
# --- MODE 9: VIBER & WHATSAPP LINK GENERATOR (INDEPENDENT INPUTS WITH REAL-TIME AUTOFILL) ---
    ...
    # Callback to handle instant auto-filling when user selects a client
    def auto_fill_viber_details():
        sel = st.session_state.get("w_viber_select_client")
        if sel and sel != "-- Select a Client --" and 'viber_client_df' in st.session_state:
            cdf = st.session_state['viber_client_df']
            name_col = st.session_state.get('viber_name_col')
            if name_col and name_col in cdf.columns:
                match = cdf[cdf[name_col] == sel]
                if not match.empty:
                    row = match.iloc[0]
                    c_name = str(row[name_col])
                    
                    proj = str(row.get('PROJECT', '')).strip() if pd.notna(row.get('PROJECT')) else ""
                    unit = str(row.get('UNIT CODE', '')).strip() if pd.notna(row.get('UNIT CODE')) else ""
                    if proj and unit: u_details = f"{unit} ({proj})"
                    elif proj: u_details = proj
                    elif unit: u_details = unit
                    else: u_details = ""
                        
                    bal = str(row.get('LUMPSUM BALANCE', '')).strip() if pd.notna(row.get('LUMPSUM BALANCE')) else ""
                    due = str(row.get('LSB DUE DATE', '')).strip() if pd.notna(row.get('LSB DUE DATE')) else ""
                    ...
                    if bal and due: b_status = f"{bal} {due_text_phrase}"
                    elif bal: b_status = bal
                    else: b_status = ""
                    ...
                    store["viber_client_name"] = c_name
                    store["viber_unit"] = u_details
                    store["viber_balance"] = b_status
                    store["viber_phone"] = raw_p

                    # Update active widget keys so UI immediately updates
                    st.session_state["w_viber_client_name"] = c_name
                    st.session_state["w_viber_unit"] = u_details
                    st.session_state["w_viber_balance"] = b_status
                    st.session_state["w_viber_phone"] = raw_p
    ...
    if sheet_url_input:
        try:
            ...
            if name_col:
                st.session_state['viber_client_df'] = client_df
                st.session_state['viber_name_col'] = name_col
                
                client_names = client_df[name_col].dropna().unique().tolist()
                st.selectbox(
                    "Select / Search Client Name to Auto-Fill:", 
                    options=["-- Select a Client --"] + client_names,
                    key="w_viber_select_client",
                    on_change=auto_fill_viber_details
                )
    ...
    col1, col2 = st.columns(2)
    with col1:
        viber_client_name = st.text_input("Client Name:", value=store["viber_client_name"], key="w_viber_client_name", on_change=save_field, args=("viber_client_name",))
    ...
    with col2:
        unit_details = st.text_input("Unit Code & Project:", value=store["viber_unit"], key="w_viber_unit", on_change=save_field, args=("viber_unit",))
    ...
    if msg_type == "Turnover & Bank Financing Follow-up":
        message_body = f"Hi {viber_client_name}, this is Kitt from Bahai Deals| Talino Group! Just following up regarding your unit at {unit_details}. You have a balance of {formatted_bal_due}. Let me know if you would like us to secure bank financing for you now to avoid penalties -- No worries, our services are absolutely free and no hidden fees. Let me know your available time. I can walk you through prequalification process. Thank you."
    elif msg_type == "Document Request Reminder":
        message_body = f"Hi {viber_client_name}, this is Kitt from Bahai Deals| Talino Group. To move forward with your bank loan application for {unit_details}, could you please send over your latest pay stubs and valid IDs? Thank you!"
    else:
        message_body = f"Hi {viber_client_name}, great news! We have an update regarding your bank financing approval for {unit_details}. Please let me know when you're available for a quick 5-minute call. Thanks!"
